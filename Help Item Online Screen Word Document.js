...
let maxLength = 200;
let processQuery = `SELECT * FROM PROCESS where PROCESS_NAME=:ONLINE_SCREEN_NAME AND FUNCTIONAL_AREA_UUID=:APP_LOGGED_IN_FUNTIONAL_AREA_ID`;
let processQueryData = await serviceOrchestrator.selectRecordsUsingQuery(
  'PRIMARYSPRINGFM',
  processQuery,
  msg.payload.apiRequestBody
);
if (processQueryData.length > 0) {
  input = Object.assign(input, processQueryData[0]);
  ...
  if (processQueryData.length > 0 && processQueryData[0].PROCESS_DESCRIPTION) {
    let desNavigation = processQueryData[0].PROCESS_DESCRIPTION;
  ...
  if (navigationFlowData.length > 0 && navigationFlowData[0].ONLINE_SCREEN_PAGE_NAVIGATION_TREE) {
    let listNavigation = formatDataToMapArray(
      JSON.parse(navigationFlowData[0].ONLINE_SCREEN_PAGE_NAVIGATION_TREE),
      null
    );
  ...
  let userActionQueryData = await serviceOrchestrator.selectRecordsUsingQuery(
    'PRIMARYSPRINGFM',
    userActionQuery,
    input
  );
  const stepDefAttributeQuery = `SELECT * FROM STEP_DEFINITION_ATTRIBUTE order by STEP_DEFINITION_ATTRIBUTE_ID asc`;
  let stepDefAttributeQueryDataList = await serviceOrchestrator.selectRecordsUsingQuery(
    'PRIMARYSPRINGFM',
    stepDefAttributeQuery,
    input
  );
  const stepDefTemplateVerbiageQuery = `SELECT * FROM STEP_DEFINITION_TEMPLATE_VERBIAGE `;
        ...
        for (let data of testCaseStepNormalQueryData) {
          if (data && data['TEST_CASE_EXECUTON_TYPE'] != 'Recorded') {
            ...
            if (stepDefAttributeQueryData && stepDefAttributeQueryData.length) {
              ...
              let actualColumnHeaderUUID = getColumnHeaderData(
                  ...
                  case 'adcf6e25-f890-476c-bdcf-e723c6d7894c':
                    {
                      ...
                      if (uiElementQueryData && Object.keys(uiElementQueryData).length) {
                        ...
                        if (uiElementQueryData['UI_ELEMENT_NAME'].toUpperCase() == 'PASSWORD') {
  ...
  let documentName = input.ONLINE_SCREEN_NAME + '_Online_Screen';
  msg.payload.result.documentName =
    documentName.length > maxLength ? documentName.substring(0, maxLength) : documentName;
  msg.payload.documentData.documentHeader = 'Online Screen is not Present';
}
