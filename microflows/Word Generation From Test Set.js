function generateBusinessRequirement(requirements) {
  ...
  requirements.forEach((req) => {
    ...
    const cos = req['Condition Of Satisfaction/Acceptance Criteria']
      ? req['Condition Of Satisfaction/Acceptance Criteria']
      : '';
    const combinedEntry = `${requirement} $$2S ${cos}`;
...
let testCaseFunctionNavigationStepQuery = `SELECT TEST_SET_ID AS 'Test Set ID', TEST_CASE_ID AS 'Test Case ID', CONCAT(TEST_CASE_STEP_SEQ_ID,'-',TEST_CASE_FUNCTION_STEP_SEQ_ID,'-',vns.VIEW_NAVIGATION_STEP_SEQ_ID) AS 'Test Case Step ID', vns.VIEW_NAVIGATION_STEP_SEQ_ID AS 'Test Case Step Seq ID', vns.VIEW_NAVIGATION_STEP_TYPE AS 'Test Case Step Type', '' AS 'Test Case Step Name', vns.VIEW_NAVIGATION_STEP_UUID AS 'Test Case Step UUID', vns.VIEW_NAVIGATION_STEP_ATTRIBUTE_KEYS AS 'ATTRIBUTE_KEYS', tc.TEST_CASE_UUID AS 'Test Case UUID' FROM VIEW_NAVIGATION_STEP vns, VIEW_NAVIGATION vs, TEST_CASE_FUNCTION_STEP tcfs, STEP_DEFINITION_TEMPLATE_VERBIAGE tcsd, TEST_CASE_STEP tcs, TEST_CASE tc, TEST_SET ts WHERE ts.TEST_SET_UUID = tcs.TEST_SET_UUID AND tc.TEST_CASE_UUID = tcs.TEST_CASE_UUID AND tcfs.TEST_CASE_STEP_UUID = tcs.TEST_CASE_STEP_UUID AND vns.STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID = tcsd.STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID AND tcs.TEST_SET_UUID = :TEST_SET_UUID AND tcfs.CURRENT_PAGE_CONTEXT = vs.PAGE_UUID AND vs.VIEW_NAVIGATION_UUID = vns.VIEW_NAVIGATION_UUID AND tcfs.IS_UI_ELEMENT_GROUP_STEP = 'No' AND tc.TEST_CASE_EXECUTON_TYPE = 'Automated' AND tcfs.IS_PURE_NAVIGATION_STEP = 'Yes' AND EXISTS( SELECT 1 FROM VIEW_NAVIGATION_STEP vns JOIN VIEW_NAVIGATION vn ON vn.VIEW_NAVIGATION_UUID = vns.VIEW_NAVIGATION_UUID WHERE vn.PAGE_UUID = tcfs.CURRENT_PAGE_CONTEXT) ORDER BY vns.VIEW_NAVIGATION_STEP_ID , vns.VIEW_NAVIGATION_STEP_SEQ_ID ASC;`;
        ...
        for (let attribute of attributeKeysList) {
          ...
          switch (keyValue[0]) {
            ...
            case 'APIAttributeName':
              {
                const apiAttributeQuery = `SELECT ATTRIBUTE_NAME FROM API_ATTRIBUTE WHERE API_ATTRIBUTE_UUID in(${keyValue[1]}) and FUNCTIONAL_AREA_UUID=:FUNCTIONAL_AREA_UUID`;
...
let requirementQuery = `SELECT tc.TEST_CASE_UUID AS 'Test Case UUID', rt.REQUIREMENT_TITLE AS 'Requirement Title',R.REQUIREMENT_TEXT as 'Requirement', COS.CONDITION_SATISFACTION_NAME as 'Condition Of Satisfaction/Acceptance Criteria' FROM TEST_SET ts INNER JOIN TEST_CASE tc ON ts.TEST_SET_UUID = tc.TEST_SET_UUID INNER JOIN TEST_CASE_REQUIREMENT tcr ON tc.TEST_CASE_UUID = tcr.TEST_CASE_UUID Left JOIN REQUIREMENT_TITLE rt ON tcr.REQUIREMENT_TITLE_UUID = rt.REQUIREMENT_TITLE_UUID Left JOIN REQUIREMENT R ON R.REQUIREMENT_UUID = tcr.REQUIREMENT_UUID Left JOIN CONDITION_SATISFACTION COS ON tcr.CONDITION_SATISFACTION_UUID = COS.CONDITION_SATISFACTION_UUID WHERE ts.TEST_SET_UUID = :TEST_SET_UUID AND ts.FUNCTIONAL_AREA_UUID =:FUNCTIONAL_AREA_UUID`;
...
let testCaseAttachmentQuery = `SELECT tc.TEST_CASE_UUID AS 'Test Case UUID', atcinfo.INFO_4 AS 'Attachment', atc.ATCHD_FILE_NM as 'File_Name' FROM TEST_SET ts INNER JOIN TEST_CASE tc ON ts.TEST_SET_UUID = tc.TEST_SET_UUID INNER JOIN ATTACHMENT atc ON tc.TEST_CASE_UUID = atc.TBL_RW_ID INNER JOIN ATTACHMENTINFO atcinfo ON atc.ATCHMT_ID = atcinfo.INFO_1 WHERE ts.TEST_SET_UUID = :TEST_SET_UUID AND atc.KEY_NM = 'TEST_CASE_UUID' AND atc.isDeleted = 0;`;
let testCaseAttachmentQueryData = await serviceOrchestrator.selectRecordsUsingQuery(
  'PRIMARYSPRINGFM',
  testCaseAttachmentQuery,
  input
);
testCaseQueryData.map((testCaseDetail, index) => {
  let dataObject = {};
  ...
  var attachmentArray = [];
  let reqNameArray = [];
  dataObject['HEADING2'] = '3.' + (index + 1) + ' ' + testCaseDetail['Test Case Name'];
  dataObject['ChildrenMap'] = [];
  for (let testCaseStepDetail of testCaseStepNormalQueryData) {
    if (
      testCaseDetail['Test Case UUID'] == testCaseStepDetail['Test Case UUID'] &&
  ...
  for (let attachment of testCaseAttachmentQueryData) {
    if (testCaseDetail['Test Case UUID'] == attachment['Test Case UUID']) {
      let newAttachment = {};
  ...
  for (let descriptions of testCaseDescriptionQueryData) {
    if (
      ...
      testCaseDetail['Test Case Execution Type'] == 'Manual'
    ) {
      testCaseDescriptionArray.push(
        descriptions['TEST_CASE_DESCRIPTION_DATA'] ? descriptions['TEST_CASE_DESCRIPTION_DATA'] : ' '
      );
      preConditionpArray.push(descriptions['TEST_CASE_PRE_CONDITION'] ? descriptions['TEST_CASE_PRE_CONDITION'] : ' ');
      userInputpArray.push(descriptions['TEST_CASE_USER_INPUT'] ? descriptions['TEST_CASE_USER_INPUT'] : ' ');
      exectedResultpArray.push(
        descriptions['TEST_CASE_EXPECTED_RESULT'] ? descriptions['TEST_CASE_EXPECTED_RESULT'] : ' '
      );
      actulResultArray.push(descriptions['TEST_CASE_ACTUAL_RESULT'] ? descriptions['TEST_CASE_ACTUAL_RESULT'] : ' ');
    } else if (
      testCaseDetail['Test Case UUID'] == descriptions['Test Case UUID'] &&
      testCaseDetail['Test Case Execution Type'] == 'Automated'
    ) {
  ...
  let subObjectDesc = { HEADING3: '3.' + (index + 1) + '.1 Test Case Description' };
  let subObjectStep = { HEADING3: '3.' + (index + 1) + '.2 Test Case Steps' };
  let subObjectAttachment = { HEADING3: '3.' + (index + 1) + '.3 Attachments' };
  let subObjectREQ = { HEADING3: '3.' + (index + 1) + '.4 Linked Requirement' };
  subObjectStep['SubChildrenMap'] = [];
  if (testCaseDetail['Test Case Execution Type'] == 'Manual') {
    let subObjectPreConditon = { HEADING4: '3.' + (index + 1) + '.2.1 Pre Condition' };
    let subObjectUserInput = { HEADING4: '3.' + (index + 1) + '.2.2 User Input' };
    let subObjectExpectedResult = { HEADING4: '3.' + (index + 1) + '.2.3 Expected Result' };
    let subObjectActualResult = { HEADING4: '3.' + (index + 1) + '.2.4 Actual Result' };
    subObjectPreConditon['SubChildList'] = preConditionpArray;
    subObjectUserInput['SubChildList'] = userInputpArray;
    subObjectExpectedResult['SubChildList'] = exectedResultpArray;
    subObjectActualResult['SubChildList'] = actulResultArray;
    subObjectStep['SubChildrenMap'].push(subObjectPreConditon);
    subObjectStep['SubChildrenMap'].push(subObjectUserInput);
    subObjectStep['SubChildrenMap'].push(subObjectExpectedResult);
    subObjectStep['SubChildrenMap'].push(subObjectActualResult);
  }
...
let requirementprocessQuery = `SELECT rt.REQUIREMENT_TITLE as 'Requirement Title', R.REQUIREMENT_TEXT as 'Requirement', COS.CONDITION_SATISFACTION_NAME as 'Condition Of Satisfaction/Acceptance Criteria' from IMPACTED_PROCESS process inner JOIN TEST_SET tset on tset.USER_ACTION_UUID = process.USER_ACTION_UUID Left JOIN REQUIREMENT_TITLE rt ON process.REQUIREMENT_TITLE_UUID = rt.REQUIREMENT_TITLE_UUID Left JOIN REQUIREMENT R ON R.REQUIREMENT_UUID = process.REQUIREMENT_UUID Left JOIN CONDITION_SATISFACTION COS ON process.CONDITION_SATISFACTION_UUID = COS.CONDITION_SATISFACTION_UUID where ASSOCIATION_TYPE in( 'REQUIREMENT', 'CONDITION_SATISFACTION', 'USER_ACTION_REQUIREMENT', 'USER_ACTION_CONDITION_SATISFACTION') and tset.TEST_SET_UUID = :TEST_SET_UUID and process.IMPACTED_PROCESS_UUID not in ( SELECT process.IMPACTED_PROCESS_UUID FROM IMPACTED_PROCESS process, TEST_CASE_REQUIREMENT tcr , TEST_CASE tc where process.IMPACTED_PROCESS_UUID = tcr.IMPACTED_PROCESS_UUID and tcr.TEST_CASE_UUID = tc.TEST_CASE_UUID and tc.TEST_SET_UUID = :TEST_SET_UUID ) Union SELECT rt.REQUIREMENT_TITLE as 'Requirement Title', R.REQUIREMENT_TEXT as 'Requirement', COS.CONDITION_SATISFACTION_NAME as 'Condition Of Satisfaction/Acceptance Criteria' from TEST_CASE_REQUIREMENT tcr Inner JOIN TEST_CASE stepUser On tcr.TEST_CASE_UUID = stepUser.TEST_CASE_UUID Inner JOIN IMPACTED_PROCESS ip ON stepUser.TEST_CASE_UUID = tcr.TEST_CASE_UUID Left JOIN REQUIREMENT_TITLE rt ON tcr.REQUIREMENT_TITLE_UUID = rt.REQUIREMENT_TITLE_UUID Left JOIN REQUIREMENT R ON R.REQUIREMENT_UUID = tcr.REQUIREMENT_UUID Left JOIN CONDITION_SATISFACTION COS ON tcr.CONDITION_SATISFACTION_UUID = COS.CONDITION_SATISFACTION_UUID where tcr.IMPACTED_PROCESS_UUID = ip.IMPACTED_PROCESS_UUID and stepUser.TEST_SET_UUID = :TEST_SET_UUID`;
let requirementprocessQueryData = await serviceOrchestrator.selectRecordsUsingQuery(
  'PRIMARYSPRINGFM',
  requirementprocessQuery,
  input
);
if (testSetQueryData && Object.keys(testSetQueryData).length) {
  msg.payload.documentData.documentHeader =
    'Test Set - ' + testSetQueryData['TEST_SET_ID'] + ' – ' + testSetQueryData['TEST_SET_NAME'];
  ...
  let userActionQueryData = await serviceOrchestrator.selectSingleRecordUsingQuery(
    `PRIMARYSPRINGFM`,
    userActionQuery,
    input
  );
  ...
  headerTwoMap['ChildrenList'] = generateBusinessRequirement(requirementprocessQueryData);
