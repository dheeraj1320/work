...
if (input[0].PAGE_UUID && (input[0].CHECK!=input[0]["PAGE_ACCESS_RELATIVE_URL"])) {
  ...
  if (input[0]["PAGE_ACCESS_RELATIVE_URL"]) {
    input[0].CHECK=input[0]["PAGE_ACCESS_RELATIVE_URL"];
    let QuerytoFetchNavigationStep = `select VIEW_NAVIGATION_STEP_UUID from  VIEW_NAVIGATION_STEP where VIEW_NAVIGATION_UUID in (select VIEW_NAVIGATION_UUID from VIEW_NAVIGATION where PAGE_UUID='${input[0]["PAGE_UUID"]}') AND FUNCTIONAL_AREA_UUID = '${input[0]["APP_LOGGED_IN_FUNTIONAL_AREA_ID"]}'`;
