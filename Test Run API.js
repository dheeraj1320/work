try {
  let input = Object.assign(msg.payload.apiRequestBody?.baseEntity?.records[0] ? msg.payload.apiRequestBody?.baseEntity?.records[0] : msg.payload.apiRequestBody, msg.payload.referenceData);
  ...
  let data = {
    ...
    INCLUDED_TEST_CASES: input.INCLUDED_TEST_CASES,
  };
  const url = env.get('AUTOMATION_URL');
  const final_url = `${url}/execute`;
  msg.payload.connectorType = 'rally';
  msg.payload.methodType = 'update';
  msg.payload.url = final_url;
