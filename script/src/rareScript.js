...
const rare = async (req, res, mainKnex, auditKnex) => {
    try {
        const functionalAreaUUIDs = ['0cf4bf15-811c-4617-8ad8-40b574c94043', 'a64e22ab-6b23-4a5b-a645-af71a8ffb2f8'];
        const functionStepResponse = await mainKnex.select('*').where({ IS_PURE_NAVIGATION_STEP: 'Yes'}).andWhere('FUNCTIONAL_AREA_UUID', 'in',functionalAreaUUIDs).from('TEST_CASE_STEP').orderBy('TEST_CASE_STEP_ID', 'asc');
