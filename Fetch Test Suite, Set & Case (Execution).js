try {
  ...
  if (input['PARENT_GRID_NAME'] == 'TEST_RUN') {
    ...
    if (input['TEST_SUITE_UUID'] && input['TEST_SUITE_UUID'].trim() !== '') {
      ...
      console.log('TEST_SUITE_UUID is missing, using fallback query with only TEST_RUN_UUID');
    ...
    if (input['selectedTestSet'] && input['selectedTestSet'] != 'null') {
      if (!input['TEST_RUN_UUID']) {
        const getTestRunQuery = ` SELECT ted.TEST_RUN_UUID FROM TEST_EXECUTION_DETAIL ted JOIN TEST_SET ts ON ted.TEST_SET_UUID = ts.TEST_SET_UUID WHERE ts.TEST_SET_NAME=:selectedTestSet AND ted.TEST_SUITE_UUID=:TEST_SUITE_UUID AND ted.FUNCTIONAL_AREA_UUID=:APP_LOGGED_IN_FUNTIONAL_AREA_ID ORDER BY ted.TEST_CASE_EXECUTION_DATE DESC LIMIT 1; `;
