try {
  ...
  /* Actual Script starts here */ const selectedFunctionalAreas = input.SELECTED_FUNCTIONAL_AREAS
    ? input.SELECTED_FUNCTIONAL_AREAS.split(',')
        ...
        .join(',')
    : `''`;
  const pageQuery = `SELECT PAGE_NAME, PAGE_UUID FROM PAGE WHERE FUNCTIONAL_AREA_UUID IN (${selectedFunctionalAreas}) ORDER BY PAGE_ID DESC LIMIT 5;`;
    ...
    for (const view of viewData) {
      ...
      if (testCaseData && testCaseData.length > 0) {
        if (testCaseData[0].TEST_CASE_NAME == updatedTcName) {
          ...
          const updateTcQuery = `UPDATE TEST_CASE SET TEST_CASE_NAME = '${updatedTcName}', AE_UPDATE_ID = '${USER_ID}', AE_UPDATE_TS= :curr_dt, TEST_CASE_STATUS = 'DRAFT' WHERE TEST_CASE_UUID = '${oldData.TEST_CASE_UUID}'`;
          ...
          const auditObj = await createAuditObject(
            { ...oldData, TEST_CASE_NAME: updatedTcName, AE_UPDATE_ID: USER_ID, AE_UPDATE_TS: formatDateToSQL(new Date()), TEST_CASE_STATUS: 'DRAFT' },
            'Update',
            oldData,
            USER_ID
          );
