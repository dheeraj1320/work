try {
  ...
  if (input && input.GRID_NAME == 'Test Case') {
    ...
    const selectQuery = `SELECT VIEW_NAVIGATION_STEP_UUID,VIEW_NAVIGATION_STEP_ID,VIEW_NAVIGATION_STEP_NAME,STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID,VIEW_NAVIGATION_STEP_TYPE,VIEW_NAVIGATION_STEP_SEQ_ID,CURRENT_PAGE_CONTEXT,NEXT_PAGE_CONTEXT,VIEW_NAVIGATION_STEP_ATTRIBUTE_KEYS,VIEW_NAVIGATION_UUID,VIEW_UUID,AE_INSERT_ID,AE_UPDATE_ID,AE_TRANSACTION_ID,AE_INSERT_TS,AE_UPDATE_TS,'No' as IS_PURE_NAVIGATION_STEP FROM VIEW_NAVIGATION_STEP WHERE VIEW_UUID=:VIEW_UUID AND FUNCTIONAL_AREA_UUID=:APP_LOGGED_IN_FUNTIONAL_AREA_ID ORDER BY VIEW_NAVIGATION_STEP_SEQ_ID asc`;
    queryData = await serviceOrchestrator.selectRecordsUsingQuery(`PRIMARYSPRINGFM`, selectQuery, input);
  }
  if (input && input.IS_PURE_NAVIGATION_STEP == 'Yes' && input.VIEW_UUID != null) {
    console.log('------IN--------');
    const selectQuery = `SELECT VIEW_NAVIGATION_STEP_UUID,VIEW_NAVIGATION_STEP_ID,VIEW_NAVIGATION_STEP_NAME,STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID,VIEW_NAVIGATION_STEP_TYPE,VIEW_NAVIGATION_STEP_SEQ_ID,CURRENT_PAGE_CONTEXT,NEXT_PAGE_CONTEXT,VIEW_NAVIGATION_STEP_ATTRIBUTE_KEYS,VIEW_NAVIGATION_UUID,VIEW_UUID,AE_INSERT_ID,AE_UPDATE_ID,AE_TRANSACTION_ID,AE_INSERT_TS,AE_UPDATE_TS,:IS_PURE_NAVIGATION_STEP as IS_PURE_NAVIGATION_STEP FROM VIEW_NAVIGATION_STEP WHERE VIEW_UUID=:VIEW_UUID AND FUNCTIONAL_AREA_UUID=:APP_LOGGED_IN_FUNTIONAL_AREA_ID ORDER BY VIEW_NAVIGATION_STEP_SEQ_ID asc`;
    queryData = await serviceOrchestrator.selectRecordsUsingQuery(`PRIMARYSPRINGFM`, selectQuery, input);
