...
try {
  ...
  let functionQueryData = await serviceOrchestrator.selectRecordsUsingQuery(
    'PRIMARYSPRINGFM',
    functionQuery,
    input
  );
  ...
  let functionStepNormalQueryData =
    await serviceOrchestrator.selectRecordsUsingQuery(
      'PRIMARYSPRINGFM',
                ...
                case 'APIAttributeName':
                  {
                    ...
                    id = apiAttributeQueryData['ATTRIBUTE_ID'];
  ...
  if (functionStepNormalQueryData && functionStepNormalQueryData.length) {
    ...
    for (let data of stepDefinitionBasedOnMode) {
      ...
      for (let uiElement of uiElementQueryData) {
        ...
        let type =
          data['stepFilter'] == 'Expected Result Step'
            ? 'Expected Result'
            : 'User Input';
        ...
        for (let stepDefData of stepDefinationData) {
          let inc = 0;
          let stepDefTemplateVerbiageId =
            stepDefData['STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID'];
          let testCaseNameStepDefVerbiageStr =
            stepDefData['Step Definition Template'];
          console.log(
            data['primaryMode'],
            ' ::::::::::::::::::::::::::: ',
            testCaseNameStepDefVerbiageStr
          );
          let stepVerviageID =
            `{StepVerbiage@#$'` + stepDefTemplateVerbiageId + `'}`;
          ...
          let valueArr = stepDefAttributeQueryData.map(function (item) {
            if (
              ...
              [
                '903cf3b0-f8a5-11ee-a163-cdf5a57b7d43',
                'adcf6e25-f890-476c-bdcf-e723c6d7894c',
              ].includes(item['STEP_DEFINITION_ATTRIBUTE_MASTER_UUID'])
            ) {
              isUIElementNameOrUserActionNameExist = true;
            } else if (
          ...
          if (isUIElementNameOrUserActionNameExist) {
              ...
              if (codeDesc['STEP_DEFINITION_ATTRIBUTE_MASTER_UUID']) {
                switch (codeDesc['STEP_DEFINITION_ATTRIBUTE_MASTER_UUID']) {
                  case '57b76ab3-8112-4343-af0f-49643c808bf7':
                    ...
                    testCaseNameStepDefVerbiageStr =
                      testCaseNameStepDefVerbiageStr.replaceAll(
                        '<Page Name>',
                        function () {
                          return (
                            `'` + testSetQueryData[0]['Test Set Name'] + `'`
                          );
                        }
                      );
                    testCaseNameAttributeKeysStepDefVerbiageStr.push(
                      '{PageName@#$' + `'` + input['PAGE_UUID'] + `'}`
                    );
                    break;
                  ...
                  case '005d158d-428c-4bca-ae2d-1c3f9630b549':
                    ...
                    testCaseNameAttributeKeysStepDefVerbiageStr.push(
                      '{EventName@#$' + `'` + uiElement['EVENT_NAME'] + `'}`
                    );
                    break;
                  ...
                  case '99cf0e40-f8a5-11ee-a163-cdf5a57b7d43':
                    {
                      ...
                      testCaseNameStepDefVerbiageStr =
                        testCaseNameStepDefVerbiageStr.replaceAll(
                          ...
                          function () {
                            return (
                              `'` + uiElement['UI_ELEMENT_TYPE_NAME'] + `'`
                            );
                          }
                        );
                      testCaseNameAttributeKeysStepDefVerbiageStr.push(
            ...
            let stepDefType =
              data['stepFilter'] == 'Expected Result Step' ? 'Then ' : 'When ';
            stepDefData['Test Case Step Name'] =
              stepDefType + testCaseNameStepDefVerbiageStr;
