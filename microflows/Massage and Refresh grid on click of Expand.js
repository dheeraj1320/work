try {
  ...
  function filterStepByCurrentStep(stepAttrValueList, currentStepKey, currentStepID, currentAttributeId) {
    let result = stepAttrValueList.filter((item) => item[currentStepKey] == currentStepID);
    if (result && result.length) {
      return result;
    } else {
      return [];
  ...
  if (input && input.FIRST_GRID == 'TEST_CASE_STEP_TAB') {
    ...
    const selectQuery = `SELECT UI_ELEMENT_GROUP_STEP_UUID,UI_ELEMENT_GROUP_STEP_SEQ_ID, UI_ELEMENT_GROUP_STEP_ID, UI_ELEMENT_GROUP_STEP_NAME, CURRENT_PAGE_CONTEXT as PAGE_NAME, STEP_TYPE, 'UI_ELEMENT_GROUP_STEP_ATTRIBUTE_VALUE' as CHILD_ATTRIBUTE_TABLE_NAME, 'UI_ELEMENT_GROUP_STEP_ATTRIBUTE_DATA' as CHILD_ATTRIBUTE_DATA, 'UI_ELEMENT_GROUP_STEP_UUID' as PRIMARY_COLUMN_NAME, UI_ELEMENT_GROUP_STEP_UUID as PRIMARY_COLUMN_VALUE, STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID FROM UI_ELEMENT_GROUP_STEP WHERE UI_ELEMENT_GROUP_UUID=:UI_ELEMENT_GROUP_UUID and FUNCTIONAL_AREA_UUID=:APP_LOGGED_IN_FUNTIONAL_AREA_ID ORDER BY UI_ELEMENT_GROUP_STEP_SEQ_ID asc`;
    queryData = await serviceOrchestrator.selectRecordsUsingQuery(`PRIMARYSPRINGFM`, selectQuery, input);
    ...
    const selectQuery = `SELECT TEST_CASE_VIEW_NAVIGATION_STEP_UUID, :TEST_CASE_STATUS as TEST_CASE_STATUS, :TEST_CASE_OWNER as TEST_CASE_OWNER, TEST_CASE_VIEW_NAVIGATION_STEP_ID, TEST_CASE_VIEW_NAVIGATION_STEP_NAME, STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID, TEST_CASE_VIEW_NAVIGATION_STEP_TYPE, TEST_CASE_VIEW_NAVIGATION_STEP_SEQ_ID, CURRENT_PAGE_CONTEXT, NEXT_PAGE_CONTEXT, 'TEST_CASE_VIEW_NAVIGATION_STEP_ATTRIBUTE_VALUE' as CHILD_ATTRIBUTE_TABLE_NAME, 'TEST_CASE_VIEW_NAVIGATION_STEP_ATTRIBUTE_DATA' as CHILD_ATTRIBUTE_DATA, 'TEST_CASE_VIEW_NAVIGATION_STEP_UUID' as PRIMARY_COLUMN_NAME, TEST_CASE_VIEW_NAVIGATION_STEP_UUID as PRIMARY_COLUMN_VALUE, VIEW_UUID, 'No' as IS_PURE_NAVIGATION_STEP FROM TEST_CASE_VIEW_NAVIGATION_STEP WHERE TEST_CASE_STEP_UUID=:TEST_CASE_STEP_UUID AND COALESCE(FUNCTION_UUID, '') = '' AND COALESCE(FUNCTION_STEP_UUID, '') = '' ORDER BY TEST_CASE_VIEW_NAVIGATION_STEP_SEQ_ID asc`;
    queryData = await serviceOrchestrator.selectRecordsUsingQuery(`PRIMARYSPRINGFM`, selectQuery, input);
  } else if (input && input.GRID_NAME == 'Test Case Function View Navigation Step') {
    const selectQuery = `SELECT TEST_CASE_VIEW_NAVIGATION_STEP_UUID, :TEST_CASE_STATUS as TEST_CASE_STATUS, :TEST_CASE_OWNER as TEST_CASE_OWNER, TEST_CASE_VIEW_NAVIGATION_STEP_ID, TEST_CASE_VIEW_NAVIGATION_STEP_NAME, STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID, TEST_CASE_VIEW_NAVIGATION_STEP_TYPE, TEST_CASE_VIEW_NAVIGATION_STEP_SEQ_ID, CURRENT_PAGE_CONTEXT, NEXT_PAGE_CONTEXT, 'TEST_CASE_VIEW_NAVIGATION_STEP_ATTRIBUTE_VALUE' as CHILD_ATTRIBUTE_TABLE_NAME, 'TEST_CASE_VIEW_NAVIGATION_STEP_ATTRIBUTE_DATA' as CHILD_ATTRIBUTE_DATA, 'TEST_CASE_VIEW_NAVIGATION_STEP_UUID' as PRIMARY_COLUMN_NAME, TEST_CASE_VIEW_NAVIGATION_STEP_UUID as PRIMARY_COLUMN_VALUE, VIEW_UUID, 'No' as IS_PURE_NAVIGATION_STEP FROM TEST_CASE_VIEW_NAVIGATION_STEP WHERE TEST_CASE_STEP_UUID=:TEST_CASE_STEP_UUID AND FUNCTION_UUID = :FUNCTION_UUID AND FUNCTION_STEP_UUID = :FUNCTION_STEP_UUID ORDER BY TEST_CASE_VIEW_NAVIGATION_STEP_SEQ_ID asc`;
    queryData = await serviceOrchestrator.selectRecordsUsingQuery(`PRIMARYSPRINGFM`, selectQuery, input);
  }
  if (input && input.GRID_NAME == 'Function Step' && input.IS_PURE_NAVIGATION_STEP == 'Yes') {
    const selectQuery = `SELECT VIEW_NAVIGATION_STEP_UUID, VIEW_NAVIGATION_STEP_ID, VIEW_NAVIGATION_STEP_NAME, STEP_DEFINITION_TEMPLATE_VERBIAGE_UUID, VIEW_NAVIGATION_STEP_TYPE, VIEW_NAVIGATION_STEP_SEQ_ID, CURRENT_PAGE_CONTEXT, NEXT_PAGE_CONTEXT, 'VIEW_NAVIGATION_STEP_ATTRIBUTE_VALUE' as CHILD_ATTRIBUTE_TABLE_NAME, 'VIEW_NAVIGATION_STEP_ATTRIBUTE_DATA' as CHILD_ATTRIBUTE_DATA, 'VIEW_NAVIGATION_STEP_UUID' as PRIMARY_COLUMN_NAME, VIEW_NAVIGATION_STEP_UUID as PRIMARY_COLUMN_VALUE, VIEW_UUID, 'No' as IS_PURE_NAVIGATION_STEP FROM VIEW_NAVIGATION_STEP WHERE VIEW_UUID=:VIEW_UUID ORDER BY VIEW_NAVIGATION_STEP_SEQ_ID asc`;
    queryData = await serviceOrchestrator.selectRecordsUsingQuery(`PRIMARYSPRINGFM`, selectQuery, input);
  }
        ...
        for (let codeDesc of stepDefAttributeQueryData) {
          switch (codeDesc['STEP_DEFINITION_ATTRIBUTE_MASTER_UUID']) {
            ...
            case '74da67d2-41c9-4cf7-9eea-715243e5fcdc':
              {
                let filteredResult = filterStepByCurrentStep(stepAttributeValueData, data['PRIMARY_COLUMN_NAME'], data['PRIMARY_COLUMN_VALUE'], codeDesc['STEP_DEFINITION_ATTRIBUTE_UUID']);
                ...
                if (uiElementValueData && Object.keys(uiElementValueData).length && uiElementValueData['SCOPE_VARIABLE_UUID']) {
                  ...
                  if (
                    ...
                    ([
                      ...
                      ['TEST_CASE_FUNCTION_STEP_ATTRIBUTE_DATA'].includes(data['REFERENCE_ATTRIBUTE_DATA']))
                  ) {
