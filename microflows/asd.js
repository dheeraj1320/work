function generateBusinessRequirement(requirements) {
    ...
    requirements.forEach((req, i) => {
        let title = req['Requirement Title'] ? 'BOLDTAG' + req['Requirement Title'] : req['Requirement'];
        let requirement = req['Requirement Title'] ? (req['Requirement'] || '').trim() : '';
        let cos = req['Condition Of Satisfaction/Acceptance Criteria'] ? req['Condition Of Satisfaction/Acceptance Criteria'] : '';
        const typ = req['ASSOC_TYPE'] === 'Linked' ? 'Linked' : '';
        if (cos && typ) {
            cos = cos + ' ' + typ;
    ...
    let filteredList = businessRequirementList.filter(item => item !== null && item !== undefined && item.trim() !== '');
    return filteredList;
};
msg.payload.result = {};
            ...
            if (attributeKeysList && attributeKeysList.length) {
                for (let attribute of attributeKeysList) {
                    ...
                    switch (keyValue[0]) {
                        ...
                        break;
                        case 'UIElementValue': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<UI Element Value>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        ...
                        break;
                        case 'EventType': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<Event Type>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        case 'ConfirmUIElementValue':
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<Confirm UI Element Value>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                            break;
                        case 'FunctionName': {
                        ...
                        case 'UIElementValue1': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<UI Element Value 1>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        ...
                        break;
                        case 'PageNumber': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<Page Number>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        case 'DataKey': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<Data Key>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        case 'DataValue': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<Data Value>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        case 'FileName': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<File Name>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        case 'DocumentParserName': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<Document Parser Name>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        ...
                        case 'APIAttributeName': {
                            const apiAttributeQuery = `SELECT ATTRIBUTE_ID,ATTRIBUTE_NAME FROM API_ATTRIBUTE WHERE API_ATTRIBUTE_UUID in(${keyValue[1]}) and FUNCTIONAL_AREA_UUID=:FUNCTIONAL_AREA_UUID`;
                        ...
                        case 'APIAttributeValue': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<API Attribute Value>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        case 'ResponseStatusCode': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<Response Status Code>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        ...
                        break;
                        case 'UIElementState': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<UI Element State>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
                        case 'Timeout': {
                            stepDefTemplateVerbiageName = stepDefTemplateVerbiageName.replaceAll('<Timeout>', keyValue[1] ? function() {
                                return keyValue[1];
                            } : `' '`);
                        }
                        break;
...
testCaseQueryData.map((testCaseDetail, index) => {
    ...
    for (let testCaseStepDetail of testCaseStepNormalQueryData) {
        if (testCaseDetail['Test Case UUID'] == testCaseStepDetail['Test Case UUID'] && testCaseDetail['Test Case Execution Type'] == 'Automated') {
            testCaseStepNameArray.push(testCaseStepDetail['Test Case Step Name']);
        }
    }
    for (let attachment of testCaseAttachmentQueryData) {
        if (testCaseDetail['Test Case UUID'] == attachment['Test Case UUID'] && !(attachment['File_Name'].endsWith('.png') || attachment['File_Name'].endsWith('.jpeg'))) {
            let newAttachment = {};
    ...
    for (let descriptions of testCaseDescriptionQueryData) {
        if (testCaseDetail['Test Case UUID'] == descriptions['Test Case UUID'] && testCaseDetail['Test Case Execution Type'] == 'Manual') {
            ...
            exectedResultpArray.push(descriptions['TEST_CASE_EXPECTED_RESULT'] ? descriptions['TEST_CASE_EXPECTED_RESULT'] : ' ');
    ...
    let subObjectDesc = {
        'HEADING3': '3.' + (index + 1) + '.1 Test Case Description'
    };
    let subObjectStep = {
        'HEADING3': '3.' + (index + 1) + '.2 Test Case Steps'
    };
    let subObjectAttachment = {
        'HEADING3': '3.' + (index + 1) + '.3 Attachments'
    };
    let subObjectREQ = {
        'HEADING3': '3.' + (index + 1) + '.4 Linked Requirement'
    };
    subObjectStep['SubChildrenMap'] = [];
    if (testCaseDetail['Test Case Execution Type'] == 'Manual') {
        let subObjectPreConditon = {
            'HEADING4': '3.' + (index + 1) + '.2.1 Pre Condition'
        };
        let subObjectUserInput = {
            'HEADING4': '3.' + (index + 1) + '.2.2 User Input'
        };
        let subObjectExpectedResult = {
            'HEADING4': '3.' + (index + 1) + '.2.3 Expected Result'
        };
        let subObjectActualResult = {
            'HEADING4': '3.' + (index + 1) + '.2.4 Actual Result'
        };
...
let requirementprocessQuery = `SELECT rt.REQUIREMENT_TITLE as 'Requirement Title', R.REQUIREMENT_TEXT as 'Requirement', COS.CONDITION_SATISFACTION_NAME as 'Condition Of Satisfaction/Acceptance Criteria' , 'Linked' as ASSOC_TYPE from TEST_CASE_REQUIREMENT tcr Inner JOIN TEST_CASE stepUser On tcr.TEST_CASE_UUID = stepUser.TEST_CASE_UUID Inner JOIN IMPACTED_PROCESS ip ON stepUser.TEST_CASE_UUID = tcr.TEST_CASE_UUID Left JOIN REQUIREMENT_TITLE rt ON tcr.REQUIREMENT_TITLE_UUID = rt.REQUIREMENT_TITLE_UUID Left JOIN REQUIREMENT R ON R.REQUIREMENT_UUID = tcr.REQUIREMENT_UUID Left JOIN CONDITION_SATISFACTION COS ON tcr.CONDITION_SATISFACTION_UUID = COS.CONDITION_SATISFACTION_UUID where tcr.IMPACTED_PROCESS_UUID = ip.IMPACTED_PROCESS_UUID and stepUser.TEST_SET_UUID =:TEST_SET_UUID Union SELECT rt.REQUIREMENT_TITLE as 'Requirement Title', R.REQUIREMENT_TEXT as 'Requirement', COS.CONDITION_SATISFACTION_NAME as 'Condition Of Satisfaction/Acceptance Criteria', 'Not Linked' as ASSOC_TYPE from IMPACTED_PROCESS process inner JOIN TEST_SET tset on tset.USER_ACTION_UUID = process.USER_ACTION_UUID Left JOIN REQUIREMENT R ON R.REQUIREMENT_UUID = process.REQUIREMENT_UUID Left JOIN REQUIREMENT_TITLE rt ON R.REQUIREMENT_ASSOCIATION_UUID = rt.REQUIREMENT_TITLE_UUID Left JOIN CONDITION_SATISFACTION COS ON process.CONDITION_SATISFACTION_UUID = COS.CONDITION_SATISFACTION_UUID where tset.TEST_SET_UUID =:TEST_SET_UUID and process.IMPACTED_PROCESS_UUID not in( SELECT process.IMPACTED_PROCESS_UUID FROM IMPACTED_PROCESS process, TEST_CASE_REQUIREMENT tcr, TEST_CASE tc where process.IMPACTED_PROCESS_UUID = tcr.IMPACTED_PROCESS_UUID and tcr.TEST_CASE_UUID = tc.TEST_CASE_UUID and tc.TEST_SET_UUID =:TEST_SET_UUID)`;
