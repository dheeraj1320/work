try {
  ...
  const getInsertAuditQuery = (auditObj) => {
    ...
    const values = fields.map((key) => auditObj[key]);
  ...
  let mode = { mode: 'Enable Message', message: 'Script Ran Successfully.' };
  async function createAuditObject(newObject, operationType = 'Insert', oldObject = null, operationPerformedBy = MY_ID) {
    ...
    const uuidQuery = `SELECT UUID() AS UNIQUE_UUID FROM PAGE LIMIT 1;`;
  ...
  ? input.SELECTED_FUNCTIONAL_AREAS
      .split(',')
      .map(area => `'${area.trim()}'`)
      .join(',')
  : `''`;

  const pageQuery = `SELECT PAGE_NAME, PAGE_UUID FROM PAGE WHERE FUNCTIONAL_AREA_UUID IN (${selectedFunctionalAreas}) ORDER BY PAGE_ID DESC LIMIT 10;`;
    ...
    for (const view of viewData) {
      ...
      if (testCaseData && testCaseData.length > 0) {
        if (testCaseData[0].TEST_CASE_NAME == updatedTcName) {
          ...
          const updateTcQuery = `UPDATE TEST_CASE SET TEST_CASE_NAME = '${updatedTcName}' WHERE TEST_CASE_UUID = '${oldData.TEST_CASE_UUID}'`;
          await serviceOrchestrator.update(updateTcQuery, input, 'PRIMARYSPRINGFM');
          const auditObj = await createAuditObject({ ...oldData, TEST_CASE_NAME: updatedTcName }, 'Update', oldData, MY_ID);
          const { PRE_EXISTING_DATA_JSON, USER_INPUT_JSON, ...finalAuditObj } = auditObj;
          await serviceOrchestrator.insert(getInsertAuditQuery(finalAuditObj), input, 'PRIMARYSPRINGFM_AUDIT', 'AE_AUDIT_UUID');
        }
      }
    }
  }
