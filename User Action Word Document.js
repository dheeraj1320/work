...
if (!input['TEST_SET_NAME']) {
  let testSetName = `SELECT TEST_SET_NAME,TEST_SET_ID FROM TEST_SET WHERE USER_ACTION_UUID=:USER_ACTION_UUID AND PROCESS_UUID=:PROCESS_UUID AND PAGE_UUID=:PAGE_UUID;`;
  testSetNameData = await serviceOrchestrator.selectSingleRecordUsingQuery('PRIMARYSPRINGFM', testSetName, input);
}
let documentName =
  input['TEST_SET_NAME'] && input['TEST_SET_ID']
    ? input['TEST_SET_ID'] + ' - ' + input['TEST_SET_NAME'].replaceAll(/[^A-Z0-9-]+/gi, '_') + '_Test_Set'
    : testSetNameData['TEST_SET_NAME']
    ? testSetNameData['TEST_SET_ID'] +
      ' - ' +
      testSetNameData['TEST_SET_NAME'].replaceAll(/[^A-Z0-9-]+/gi, '_') +
      '_User_Action'
    : input['USER_ACTION_NAME']
    ? input['USER_ACTION_NAME'].replaceAll(/[^A-Z0-9-]+/gi, '_') + '_User_Action'
    : 'User_Action';
...
msg.payload.documentData.documentHeader =
  input['TEST_SET_NAME'] && input['TEST_SET_ID']
    ? input['TEST_SET_ID'] + ' - ' + input['TEST_SET_NAME']
    : testSetNameData['TEST_SET_NAME']
    ? testSetNameData['TEST_SET_ID'] + ' - ' + testSetNameData['TEST_SET_NAME']
    : input['USER_ACTION_NAME']
    ? input['USER_ACTION_NAME']
    : 'User_Action';
...
if (busiReqUeryDataList.length > 0) {
  ...
  businessReq['ChildrenList'] = await formatRequirements(busiReqUeryDataList, true);
  dataOneObject['ChildrenMap'].push(businessReq);
  innerCount++;
}
let userActionTreeQuery = `SELECT USER_ACTION_UUID, USER_ACTION_REQUIREMENT_TREE FROM USER_ACTION_REQUIREMENT_TREE where USER_ACTION_UUID='${input['USER_ACTION_UUID']}'`;
...
if (userActionTreeQueryData && userActionTreeQueryData.USER_ACTION_REQUIREMENT_TREE) {
  let uaTreeData = formatDataToArray(JSON.parse(userActionTreeQueryData.USER_ACTION_REQUIREMENT_TREE), null);
  if (uaTreeData.length > 0) {
    ...
    let pageEventReq = { HEADING3: '2.' + innerCount + ' Page-Event Requirement' };

    pageEventReq['ChildrenList'] = formatDataToArray(finalTreeData, null);
...
if (testSetQueryData && testSetQueryData.length > 0) {
  ...
  for (const [i, testCaseDetail] of testCaseQueryData.entries()) {
    ...
    if (reqNameArray && reqNameArray.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectREQ);
      dataIndex++;
    }
    if (testCaseDescriptionArray && testCaseDescriptionArray.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectDesc);
      dataIndex++;
    }
    if (preExistingArray && preExistingArray.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectPreExisting);
      dataIndex++;
    }
    if (preConditionpArray && preConditionpArray.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectPreConditon);
      dataIndex++;
    }
    if (userInputpArray && userInputpArray.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectUserInput);
      dataIndex++;
    }
    if (exectedResultpArray && exectedResultpArray.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectExpectedResult);
      dataIndex++;
    }
    if (actulResultArray && actulResultArray.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectActualResult);
      dataIndex++;
    }
    if (testCaseStepNameArray && testCaseStepNameArray.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectStep);
      dataIndex++;
    }
    if (testCaseAttachmentQueryData && testCaseAttachmentQueryData.length > 0) {
      ...
      dataObject1['ChildrenList'] = dataObject1['ChildrenList'].concat(subObjectAttachment);
      dataObject1['LinkChildList'] = attachmentArray;
      dataObject1['attachmentParentKeyList'] = parentKeyList;
      dataIndex++;
    }
    dataThreeObject['ChildrenMap'].push(dataObject1);
  }
  headerTestCase['ChildrenMapList'].push(dataThreeObject);
