...
try {
  ...
  if (input['RUN_AUTOMATION_SOURCE_TYPE'] == 'NAVIGATION_STEPS') {
    selectQuery = `SELECT 'TEST_EXECUTION_DETAIL' AS TABLE_NAME, 'TEST_EXECUTION_DETAIL_UUID' AS KEY_NAME, 'TEST_CASE' AS GRID_NAME, 'VIEW_NAVIGATION' AS DOWNLOAD_NAME, NULL AS CHILD_STEP_TYPE, ted.TEST_CASE_STEP_SEQ_ID AS VIEW_NAVIGATION_STEP_ID, ted.TEST_CASE_STEP_NAME AS VIEW_NAVIGATION_STEP_NAME, ted.TEST_EXECUTION_DETAIL_UUID AS KEY_VALUE, vns.VIEW_NAVIGATION_STEP_UUID, ted.TEST_CASE_STEP_SEQ_ID, vns.VIEW_UUID, pv.PAGE_UUID, ted.TEST_RUN_UUID, ted.TEST_CASE_UUID, ted.TEST_CASE_STEP_UUID, ted.TEST_CASE_STEP_PATH, ted.TEST_CASE_STEP_EXECUTION_DATE AS EXECUTED_ON, ROUND(ted.TEST_CASE_STEP_DURATION / 1000.0, 3) AS EXECUTION_DURATION, ted.TEST_CASE_STEP_EXECUTION_STATUS AS EXECUTION_STATUS FROM TEST_EXECUTION_DETAIL ted JOIN VIEW_NAVIGATION_STEP vns ON ted.TEST_CASE_STEP_UUID = vns.VIEW_NAVIGATION_STEP_UUID JOIN PAGE_VIEW pv ON vns.VIEW_UUID = pv.VIEW_UUID WHERE ted.TEST_CASE_UUID = vns.VIEW_UUID AND ted.TEST_SET_UUID = pv.PAGE_UUID AND ted.TEST_RUN_UUID = :TEST_RUN_UUID ORDER BY ted.TEST_CASE_STEP_EXECUTION_DATE ASC, ted.TEST_CASE_STEP_SEQ_ID ASC;`;
