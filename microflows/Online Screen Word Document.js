...
msg.payload.result.documentName = input.PROCESS_NAME ? input.PROCESS_NAME : 'OnlineScreen';
...
if (navigationFlowData.length > 0 && navigationFlowData[0].ONLINE_SCREEN_PAGE_NAVIGATION_TREE) {
  let listNavigation = formatDataToArray(JSON.parse(navigationFlowData[0].ONLINE_SCREEN_PAGE_NAVIGATION_TREE), null);
  headerOneMap['ChildrenList'] = listNavigation;
}
msg.payload.documentData.documentDetails.push(headerOneMap);
let headerTwoMap = {};
headerTwoMap['Header'] = '3. User Action ';
headerTwoMap['ChildrenMapList'] = [];
let userActionQuery = `SELECT pag.PAGE_UUID,pag.PAGE_ID,vus.USER_ACTION_UUID,pag.PAGE_NAME,vus.USER_ACTION_NAME FROM PAGE pag, PROCESS_PAGE proPag, VIEW_USER_ACTION vus where pag.PAGE_UUID = vus.PAGE_UUID and vus.PROCESS_UUID=proPag.PROCESS_UUID and pag.PAGE_UUID = proPag.PAGE_UUID and proPag.PROCESS_UUID=:PROCESS_UUID order by pag.PAGE_ID`;
...
for (let userActionData of userActionQueryData) {
  let dataObject = {};
  dataObject['HEADING2'] =
    '3.' + usaIndex + ' ' + userActionData.PAGE_NAME + ' Page - ' + userActionData.USER_ACTION_NAME + ' User Action';
  dataObject['ChildrenMap'] = [];
  let objReq = { HEADING3: '3.' + usaIndex + '.1 Requirements', SubChildrenMap: [] };
  let innerCount = 1;
  let busiReqUery = `SELECT ts.USER_ACTION_UUID AS 'USER_ACTION_UUID', rt.REQUIREMENT_TITLE AS 'Requirement Title', R.REQUIREMENT_TEXT as 'Requirement', COS.CONDITION_SATISFACTION_NAME as 'Condition Of Satisfaction/Acceptance Criteria' FROM IMPACTED_PROCESS ts Left JOIN REQUIREMENT_TITLE rt ON rt.REQUIREMENT_TITLE_UUID = ts.REQUIREMENT_TITLE_UUID Left JOIN REQUIREMENT R ON R.REQUIREMENT_UUID = ts.REQUIREMENT_UUID Left JOIN CONDITION_SATISFACTION COS ON COS.CONDITION_SATISFACTION_UUID = ts.CONDITION_SATISFACTION_UUID WHERE ts.USER_ACTION_UUID='${userActionData['USER_ACTION_UUID']}'and ts.PROCESS_UUID='${input['PROCESS_UUID']}' and ASSOCIATION_TYPE in ('REQUIREMENT','CONDITION_SATISFACTION');`;
  ...
  if (busiReqUeryDataList.length > 0) {
    ...
    businessReq['SubChildList'] = generateBusinessRequirement(busiReqUeryDataList);
  ...
  if (testSetQueryData && testSetQueryData.length > 0) {
    let testCases = { HEADING3: '3.' + usaIndex + '.' + innerCount + ' Test Cases' };
    ...
    testCaseQueryData.map((testCaseDetail, i) => {
      ...
      if (testCaseStepNameArray && testCaseStepNameArray.length > 0) {
        let subObjectStep = {};
        let heading = 'BOLDTAG' + dataIndex + '. Test Case Steps';
      ...
      headerTwoMap['ChildrenMapList'].push(dataObject1);
    });
  } else {
    headerTwoMap['ChildrenMapList'].push(dataObject);
  }
