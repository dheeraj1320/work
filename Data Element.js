...
if (input.compositeEntityAction == 'Delete') {
  deleteRecord(
    'DATA_ELEMENT_UUID',
    input['DATA_ELEMENT_UUID'],
    'DATA_ELEMENT',
    input['APP_LOGGED_IN_FUNTIONAL_AREA_ID']
  );

  let QuerytoFetchDataElementsPreDefinedValues =
    'select CODE_SET_TRANSACTION_DATA_ELEMENT_UUID from CODE_SET_TRANSACTION_DATA_ELEMENT where DATA_ELEMENT_UUID=:DATA_ELEMENT_UUID;';
  let QuerytoFetchDataElementsPreDefinedValuesData = await serviceOrchestrator.selectRecordsUsingQuery(
    'INFO_TENANT',
    QuerytoFetchDataElementsPreDefinedValues,
    input
  );
  let QuerytoFetchDataElementsPreDefinedValuesRecords = JSON.parse(
    JSON.stringify(QuerytoFetchDataElementsPreDefinedValuesData)
  );
  for (let key in QuerytoFetchDataElementsPreDefinedValuesRecords) {
    deleteRecord(
      'CODE_SET_TRANSACTION_DATA_ELEMENT_UUID',
      QuerytoFetchDataElementsPreDefinedValuesRecords[key].CODE_SET_TRANSACTION_DATA_ELEMENT_UUID,
      'CODE_SET_TRANSACTION_DATA_ELEMENT',
      input['APP_LOGGED_IN_FUNTIONAL_AREA_ID']
    );
  }

  let QuerytoFetchDataElementsLongtext =
    'select DATA_ELEMENT_LONGTEXT_UUID  from DATA_ELEMENT_LONGTEXT  where DATA_ELEMENT_UUID=:DATA_ELEMENT_UUID;';
  let QuerytoFetchDataElementsLongtextData = await serviceOrchestrator.selectRecordsUsingQuery(
    'PRIMARYSPRINGFM',
    QuerytoFetchDataElementsLongtext,
    input
  );
  ...
  for (let key in QuerytoFetchDataElementsLongtextRecords) {
    deleteRecord(
      'DATA_ELEMENT_LONGTEXT_UUID',
      QuerytoFetchDataElementsLongtextRecords[key].DATA_ELEMENT_LONGTEXT_UUID,
      'DATA_ELEMENT_LONGTEXT',
      input['APP_LOGGED_IN_FUNTIONAL_AREA_ID']
    );
  }

  if(input.UI_ELEMENT_EXISTS && input.UI_ELEMENT_EXISTS === 'Yes'){
    // const queryForUIElement = `SELECT UI_ELEMENT_UUID, DATA_SET_UUID, DATA_ELEMENT_UUID FROM UI_ELEMENT WHERE DATA_ELEMENT_UUID = '${input.DATA_ELEMENT_UUID}' AND DATA_SET_UUID = '${input.DATA_SET_UUID}'`;
    const queryForUIElement = `SELECT * FROM UI_ELEMENT WHERE DATA_ELEMENT_UUID = '${input.DATA_ELEMENT_UUID}' AND DATA_SET_UUID = '${input.DATA_SET_UUID}'`;

    const queryForUIElementData =  await serviceOrchestrator.selectRecordsUsingQuery('PRIMARYSPRINGFM', queryForUIElement, input);
...
if (input.compositeEntityAction == 'Update') {
  if (
    input['AppEngChildEntity:DATA_ELEMENT_LONGTEXT'] &&
    input['AppEngChildEntity:DATA_ELEMENT_LONGTEXT'][0].DATA_ELEMENT_DEFINITION &&
    input['AppEngChildEntity:DATA_ELEMENT_LONGTEXT'][0].DATA_ELEMENT_LONGTEXT_UUID == null
  ) {
  ...
  if(input.DATA_ELEMENT_LONGTEXT_UUID && input.DATA_ELEMENT_DEFINITION && input.UI_ELEMENT_EXISTS === 'Yes'){
    const longObj = {
      DATA_ELEMENT_LONGTEXT_UUID: input.DATA_ELEMENT_LONGTEXT_UUID,
      DATA_ELEMENT_DEFINITION: input.DATA_ELEMENT_DEFINITION
    }
    DATA_ELEMENT_LONGTEXT.push(longObj);
...
let queryToFetchDataElementsData = await serviceOrchestrator.selectRecordsUsingQuery(
  'INFO_TENANT',
  queryToFetchMasterDatasetMapping,
  input
);
...
if (input.compositeEntityAction === 'Update' && input.PRE_DEFINED_DATA === 'Yes') {
  let QuerytoFetchmasterdata =
    'SELECT * FROM CODE_SET_TRANSACTION_DATA_ELEMENT WHERE DATA_ELEMENT_UUID=:DATA_ELEMENT_UUID';
  let QuerytoFetchDataElementsData = await serviceOrchestrator.selectRecordsUsingQuery(
    'INFO_TENANT',
    QuerytoFetchmasterdata,
    input
  );
