try {
  ...
  for (const testCase of testCaseData) {
    ...
    const auditObj = await createAuditObject({ ...oldData, AE_UPDATE_ID: USER_ID, AE_UPDATE_TS: formatDateToSQL(new Date()), TEST_CASE_EXECUTON_TYPE: 'Automated' }, 'Update', oldData, USER_ID);
